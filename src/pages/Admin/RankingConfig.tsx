  ...
  useEffect(() => {
    ...
    const fetchConfig = async () => {
      ...
      if (error) {
        ...
        // Config should already exist from migration
        setConfig({
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="course_completion_points">Pontos por Curso Concluído</Label>
                <Input 
                  id="course_completion_points" 
                  name="course_completion_points" 
                  type="number" 
                  value={config.course_completion_points || ''} 
                  onChange={handleInputChange} 
                  className="bg-surface" 
                />
                <p className="text-xs text-muted-foreground">Pontos base que serão divididos pelo número de aulas do curso.</p>
              </div>
              <div className="space-y-2">
                <Label htmlFor="rewatch_course_points">Pontos por Reassistir Curso</Label>
                <Input 
                  id="rewatch_course_points" 
                  name="rewatch_course_points" 
                  type="number" 
                  value={config.rewatch_course_points || ''} 
                  onChange={handleInputChange} 
                  className="bg-surface" 
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="podcast_access_points">Pontos por Acessar Podcast</Label>
                <Input 
                  id="podcast_access_points" 
                  name="podcast_access_points" 
                  type="number" 
                  value={config.podcast_access_points || ''} 
                  onChange={handleInputChange} 
                  className="bg-surface" 
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="book_access_points">Pontos por Acessar Livro</Label>
                <Input 
                  id="book_access_points" 
                  name="book_access_points" 
                  type="number" 
                  value={config.book_access_points || ''} 
                  onChange={handleInputChange} 
                  className="bg-surface" 
                />
              </div>
              <div className="flex justify-end">
                <Button type="submit" variant="netflix" disabled={isSaving}>
